...
public class WheelOptions<T> {
    ...
    private List<Area> list1;
    private List<Area> list2;
    private List<Area> list3;
    ...
    public void setData(final List<? extends Area> items) {
        // 选项1
        ...
        for (Area area : items)
            if (area.getParentId() == 0)
                list1.add(area);
        wv_option1.setAdapter(new ArrayWheelAdapter(list1));// 设置显示数据
        // 选项2
        wv_option2 = (WheelView) view.findViewById(R.id.options2);
        list2 = new ArrayList<>();
        if (list1.size() > 0) {
            ...
            for (Area area : items)
                if (area.getParentId() == id)
                    list2.add(area);
        }
        wv_option2.setAdapter(new ArrayWheelAdapter(list2));// 设置显示数据
        // 选项3
        wv_option3 = (WheelView) view.findViewById(R.id.options3);
        list3 = new ArrayList<>();
        if (list2.size() > 0) {
        ...
        wv_option3.setAdapter(new ArrayWheelAdapter(list3));// 设置显示数据
        ...
        OnItemSelectedListener wheelListener_option1 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                list2 = new ArrayList<>();
                list3 = new ArrayList<>();
                int id = list1.get(wv_option1.getCurrentItem()).getId();
                for (Area area : items)
                    if (area.getParentId() == id)
                        list2.add(area);
        ...
        OnItemSelectedListener wheelListener_option2 = new OnItemSelectedListener() {

            @Override
            public void onItemSelected(int index) {
                list3 = new ArrayList<>();
                int id = list2.get(wv_option2.getCurrentItem()).getId();
                for (Area area : items)
                    if (area.getParentId() == id)
                        list3.add(area);
                wv_option3.setAdapter(new ArrayWheelAdapter(list3));
