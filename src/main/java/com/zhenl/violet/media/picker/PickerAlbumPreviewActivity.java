...
import com.zhenl.violet.utils.BitmapDecoder;
import com.zhenl.violet.utils.FileUtil;
import com.zhenl.violet.utils.ImageUtil;
...
public class PickerAlbumPreviewActivity extends UI implements OnClickListener, ViewPager.OnPageChangeListener {
    ...
    public void setImageView(PhotoInfo info) {
        ...
        Bitmap bitmap = BitmapDecoder.decodeSampledForDisplay(getApplication(), info.getAbsolutePath());
        if (bitmap == null) {
            Toast.makeText(this, getString(R.string.picker_image_error), Toast.LENGTH_LONG).show();
        } else {
            try {
    ...
    private void updateOriImageSizeTip(boolean isOri) {
        ...
        if (isOri) {
            ...
            originalImageSizeTip.setText(String.format(this.getResources().getString(
                    R.string.picker_image_preview_original_select), FileUtil.getFileSizeString(totalSize)));
